...
public class UserLocationService {
	...
	public static LoggerManager logger = GenericUtility.getLogger(UserLocationService.class.getName());
	...
	public WSBaseResponse logout(WSUserLocationRequest request){
		...
		try{
			if (null != request){
				...
				IBusinessAction action = ActionFactory.getActionInstance(ActionDirectory.ACTION_REFRESHLOCATION);
				response = action.execute(request, ERequestType.USER_REFRESH_LOCATION);
				if (null != response) {
			...
			logger.debug("|~| User Location Service |~| Business Exception occurred during the serivce execution : " + bEx.getMessage());
			...
			logger.debug("|~| User Location Service |~| Bad Request Exception occurred during the serivce execution : " + brEx.getMessage());
			...
			logger.debug("|~| User Location Service |~| Application Exception occurred during the serivce execution : " + aEx.getMessage());
			...
			logger.debug("|~| User Location Service |~| Exception occurred during the serivce execution : " + ex.getMessage());
			...
			logger.debug("|~| User Location Service |~| Throwable occurred during the serivce execution : " + th.getMessage());
